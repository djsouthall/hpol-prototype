...
#Taken from https://www.cfa.harvard.edu/~jconnors/documents/pySmith.py
...
def get_smith(fig, rect = 111, plot_impedance = True, plot_ticks = False, plot_admittance = False, plot_labels = False):
    ...
    try:
        ...
        thetas = np.linspace(0.0, math.pi*(1-2.0/num_thetas),num_thetas//2)
        angle_ticks = []#(0, r"$0$"),
        for theta in thetas:
            angle_info = []
        ...
        thetas2 = np.linspace(math.pi, 2*math.pi*(1-1.0/num_thetas), num_thetas//2)
        angle_ticks2 = []#(0, r"$0$"),
